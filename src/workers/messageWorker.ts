...
export function createMessageWorker(
  ...
  const worker = new Worker<MessageJob>(
    ...
    async (job: Job<MessageJob>) => {
      ...
      if (senderIds && senderIds.length > 0) {
        ...
        for (const senderId of senderIds) {
          ...
          if (
            s &&
            s.status === "connected" &&
            (await senderManager.hasAvailableQuota(s))
          ) {
            sender = s;
            break;
          }
      ...
      // Get contact info
      const contactRepo = AppDataSource.getRepository(
        require("../entities/CampaignContact").CampaignContact
      );
      ...
      // Get WhatsApp client
      const client = (whatsappManager as any).clients.get(sender.id);
      if (!client) {
      ...
      try {
        ...
        const typingDuration =
          baseTypingTime + Math.random() * (2000 - 500) + 500; // Add 0.5-2s jitter
        ...
        await campaignManager.updateContactStatus(
          contactId,
          "failed",
          error.message
        );
  ...
  worker.on("failed", async (job, error) => {
    if (job) {
      ...
      if (job.attemptsMade < 3) {
        ...
        console.log(
          `🔄 Retrying job ${job.id} in ${delay}ms (attempt ${
            job.attemptsMade + 1
          }/3)`
        );
      } else {
